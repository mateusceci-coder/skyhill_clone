...
export default function ShoppingCart({ products, setProducts }) {
  ...
  const handleIncrease = (id:boolean) => {
    setProducts((product) => product.map((product) => product.id === id ? { ...product, quantity: product.quantity + 1 } : product))
  }

  const handleDeleteProduct = () => {
      setProducts((products) => products.filter((product) => product.id !== product.id));
  }
    ...
    <div>
      ...
      <Drawer open={openCart} onClose={toggleDrawer(false)} anchor="right">
        <div className="w-[320px] md:w-112 flex flex-col p-4">
          ...
          {products.length > 0 && <ul className="flex justify-between">
          ...
          {products.length === 0 && <p className="text-red-500">O CARRINHO DE COMPRAS ESTÁ VAZIO.</p> }
        </div>
      </Drawer>
    </div>
  );
